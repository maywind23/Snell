...
prompt_admin_port() {
    local default_port="${1:-$DEFAULT_ADMIN_PORT}"
    local response
    if [[ -t 0 && -t 1 ]]; then
        while true; do
            read -r -p "Enter admin web port [${default_port}]: " response || response=""
            response=${response:-$default_port}
            if validate_port "$response"; then
                echo "$response"
                return
            fi
            log_warn "Invalid port. Please enter a number between 1 and 65535."
        done
    else
        echo "$default_port"
    fi
...
setup_admin_service() {
    ...
    if [[ -n "$requested_port" ]]; then
        if ! validate_port "$requested_port"; then
            log_error "Invalid admin port: $requested_port"
            exit 1
        fi
        admin_port=$requested_port
    fi
    admin_port=${admin_port:-$DEFAULT_ADMIN_PORT}
    admin_token=${admin_token:-$(random_token)}
    server_host=${server_host:-$(hostname -I | awk '{print $1}')}
...
install_stack() {
    local requested_admin_port="${1:-}"
    local default_admin_port="$DEFAULT_ADMIN_PORT"
    if [[ -z "$requested_admin_port" && -f "$ADMIN_CONFIG" ]]; then
        # shellcheck disable=SC1090
        source "$ADMIN_CONFIG"
        default_admin_port=${ADMIN_PORT:-$DEFAULT_ADMIN_PORT}
    fi
    require_root
    install_dependencies
    ensure_directories
    ...
    if [[ -z "$resolved_admin_port" ]]; then
        resolved_admin_port=$(prompt_admin_port "$default_admin_port")
    fi
    setup_admin_service "$resolved_admin_port"
...
cli_usage() {
    cat <<'USAGE'
Usage: ./Snell.sh <command> [options]

Commands:
  install          Install Snell server and web interface
                   Options:
                     --admin-port <port>  Override admin panel listening port
        ...
        install)
            while [[ $# -gt 0 ]]; do
                case "$1" in
                    --admin-port)
                        if [[ $# -lt 2 ]]; then
                            log_error "--admin-port requires a value"
                            exit 1
                        fi
                        admin_port_arg="$2"
                        shift 2
            ...
            if [[ -n "$admin_port_arg" ]] && ! validate_port "$admin_port_arg"; then
                log_error "Invalid admin port: $admin_port_arg"
                exit 1
            fi
            install_stack "$admin_port_arg"
            ;;
        uninstall)
            uninstall_stack
            ;;
